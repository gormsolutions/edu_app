...
class MtnMomoPayTest(Document):
	def before_save(self): 
		# frappe.msgprint(_(f"Thanks your Data has been sent to Efris <br> {ocp_user}<br>{ocp}" ))
		...
		# frappe.msgprint(_(f"Thanks your Data has been sent to Efris <br> {access_token}<br>" ))
		# end of creating atoken	
		...
		# Print the response
		frappe.msgprint(_(f"A Request has been sent to {self.phone_number} Please Approve" + response.text))
		# print(response.text)
	

	def on_submit(self):
		# Set the endpoint URL and headers
		ocp = frappe.get_list("Momo User Settings", ["ocp_user"])[0]['ocp_user']
		...
		frappe.msgprint(_(f"Thanks your {self.item} on {self.phone_number} been sent" + json.dumps(response.json())))
